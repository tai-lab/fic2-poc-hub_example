...
def _update_configuration(cfg, logger):
    path = os.getenv('HUB_API_CONF_FILEPATH')
    if path is not None and os.path.isfile(path):
        logger.info(' * Updating configuration with {}'.format(path))
        cfg.update_yaml(path)
    else:
        logger.info(' * Ignoring configuration {}={}'.format('HUB_API_CONF_FILEPATH', path))

def _load_configuration(logger):
    logger.info(' * Configuration ...')
    conf = lya.AttrDict.from_yaml(resource_stream('hub.resources', 'hub.default.yml'))
